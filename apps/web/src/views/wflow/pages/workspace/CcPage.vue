<script setup>
import {getCcMeInst} from "@/api/instance.js";
...
onMounted(getCcMeInstList)

function getCcMeInstList() {
  loading.value = true
  const _params = Object.assign({}, params)
  _params.startRange = String(_params.startRange || [])
  getCcMeInst(_params).then(res => {
    loading.value = false
...
<template>
  <div>
    <search-tools :show-type="!pickerMode" start-desc="流程发起" @search="getCcMeInstList" v-model="params"/>
    <div class="w-card-sm" style="margin-top: 10px" v-loading="loading">
      <el-table :data="datas.records" :cell-style="{padding: '5px 0'}" :header-cell-style="tbCellStyle">
        ...
        <el-table-column min-width="120" prop="initiator" label="发起人">
        ...
        <el-table-column show-overflow-tooltip prop="endTime" label="结束时间"></el-table-column>
      ...
      <process-inst-preview ref="instPreview" @change="getCcMeInstList"/>
      <div style="margin-top: 20px; display: flex; justify-content: right">
        <el-pagination v-model:current-page="params.pageNo"
                       ...
                       @size-change="getCcMeInstList"
                       @current-change="getCcMeInstList"/>
      </div>
    </div>
  </div>
</template>
