...
import {getRandNodeId, reloadNodeId} from "@/utils/ProcessUtil.js";
...
function deleteBranch(i) {
  if (props.modelValue.branch.length <= 2) {
    //只有两个分支，那么就直接删除整个网关及聚合节点
    props.branch.splice(props.index, 2)
  } else {
    //直接删除此分支
...
function copyBranchs(i) {
  const brNode = _value.value.props.branch[i]
  const brNodes = _value.value.branch[i]
  sessionStorage.setItem('copyNode', JSON.stringify({brNode, brNodes}))
  ElMessage.success('复制分支成功')
}

function copyGateway() {
  sessionStorage.setItem('copyNode', JSON.stringify(_value.value))
  ElMessage.success('复制网关成功')
}

function pasteBranch() {
  const node = sessionStorage.getItem('copyNode')
  if (node) {
    const {brNode, brNodes} = JSON.parse(node)
    if (brNode && brNodes) {
      brNode.name = brNode.name + '-copy'
      reloadNodeId(brNode)
      reloadNodeId(brNodes)
      const insertIndex = _value.value.props.branch.length - 1
      _value.value.props.branch.splice(insertIndex, 0, brNode)
      _value.value.branch.splice(insertIndex, 0, brNodes)
      nodeChange()
    } else ElMessage.warning('当前已复制内容非支路格式')
  } else {
...
<template>
  <div class="w-p-gateway">
    <contextmenu ref="menu" v-if="!readonly">
      <contextmenu-item @click="copyGateway">复制网关</contextmenu-item>
      <contextmenu-item @click="pasteBranch">粘贴分支</contextmenu-item>
    </contextmenu>
    <div class="w-p-branch">
      <el-button v-contextmenu:menu class="w-p-gateway-add" round plain v-if="!readonly" @click="addBranch">添加分支</el-button>
      <el-text style="position: absolute; top: 15px" size="small" v-if="isDebug && _value.id" type="warning">{{_value.id}}</el-text>
      <!--分支遍历-->
      <div v-for="(nodes, i) in modelValue.branch" :key="i + '_br'"
        ...
        <div>
          <!--构造分支头节点-->
          <component :ref="setChildRef" :readonly="readonly" v-model="modelValue.props.branch[i]" :index="i"
...
.w-p-branch {
  ...
  &:before {
    content: '';
    width: calc(100% - var(--w-node-width));
    background: var(--el-border-color-darker);
    position: absolute;
    height: var(--w-node-line-width);
  }
