...
const butStyle = computed(() => {
  ...
  switch (prop.mode){
    case 'AUTO_REFUSE':
      return {icon: 'Close', type: 'danger', disable: true}
    case 'AUTO_PASS':
      return {icon: 'Check', type: 'success', disable: true}
    case 'USER':
      return {icon: 'Plus', type: 'primary', disable: false}
  }
})
      ...
      switch (prop.ruleType){
        ...
        case 'FORM_USER':
          if (!prop.formUser){
            ...
            desc = `表单[${prop.formUser?.name}]内人员审批`
          }
          break
        case 'NODE_SELECT':
        ...
        case 'LEADER':
          ...
          break
        case 'LEADER_TOP':
          desc = (prop.leaderTop.toEnd ? '逐级' : `前${prop.leaderTop.level}级`) + '部门主管审批'
          break
        case 'SUPERIOR':
          ...
          break
        case 'SUPERIOR_TOP':
          desc = (prop.superiorTop.toEnd ? '逐级' : `前${prop.superiorTop.level}级`) + '上级审批'
          break
        case 'ASSIGN_DEPT':
        ...
        case 'ASSIGN_ROLE':
          ...
          break
        case 'ASSIGN_GROUP':
          desc = `用户组[${prop.assignGroup.map(v => v.name).join('、')}]内的人员审批`
          break
        case 'DYNAMIC':
...
function validate(errs){
  ...
  if (prop.mode === 'USER'){
    switch (prop.ruleType){
      ...
      case 'NODE_SELECT':
        if (prop.nodeAssign.nodeIds.length === 0){
          errInfo.value = '未指定上游审批节点'
          errs.push(`${_value.value.name} 节点未指定上游审批节点`)
          return
        }
        break
      case 'ASSIGN_DEPT':
    ...
    if (prop.rejectRule?.type === 'SKIP' && !prop.rejectRule?.target){
      errInfo.value = '未指定驳回跳转节点'
      errs.push(`${_value.value.name} 节点未指定驳回跳转节点`)
      return
    }
  }
  showErr.value = false
...
<node v-model="_value" :readonly="readonly" :show-error="showErr"
      ...
      :insert-btn-icon="butStyle.icon" :insert-btn-type="butStyle.type"
      :disable-add="butStyle.disable" @paste="$emit('paste')"
