...
import { getCcMeInst } from '@/api/instance';
...
export const CcPage: React.FC = () => {
  ...
  const loadList = useCallback(async (p: number, s: number) => {
    setLoading(true);
    try {
      const res = await getCcMeInst({ pageNo: p, pageSize: s, ...paramsRef.current });
      setRecords(res.data?.records || []);
      setTotal(res.data?.total || 0);
    } catch (e: any) {
      message.error(e?.msg || t('workspace.cc.loadFailed'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns: TableColumnsType<any> = [
    ...
    { title: t('workspace.table.ccNode'), dataIndex: 'currentNodeName', key: 'currentNodeName', width: 130 },
    {
    ...
    { title: t('workspace.table.endTime'), dataIndex: 'endTime', key: 'endTime', width: 170 },
    {
